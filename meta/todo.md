...
- [ ] 继续完善 `web-preview` 的业务数据覆盖范围：
  ...
  - 补更多设置项状态组合
  - 补更多 compare 边界场景
- [x] 增加对“WAV 头损坏但 PCM payload 仍在”的容错支持
...
- [x] 增加 ffmpeg-backed 压缩音频解码支持
  - 已支持：`mp3` / `aac` / `m4a`
  - 路径：上传后先通过 ffmpeg 解码为标准 WAV，再进入现有模型预处理链路
  - 验收：补真实压缩音频 fixture、API 上传测试、前端格式提示测试
  - 暂不把 `wmv` / `flv` 写入产品承诺；后端虽能按视频扩展名识别，但还缺真实容器 fixture 与浏览器侧验收
